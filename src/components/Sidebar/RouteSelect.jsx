...
const RouteSelect = () => {
  return (
    <div className="space-y-5 p-4">
      ...
      <Route selected={false} icon="CreditCard" iconName="Payment" />
      <Route
        selected={false}
        icon="ClipboardDocumentCheck"
        iconName="Settlement"
      />
      <Route selected={false} icon="UserGroup" iconName="Team" />
...
const Route = ({ selected, icon, iconName }) => {
  return (
    <button className={`${selected ? "bg-white text-stone-950 shadow-lg" : "text-stone-500 hover:bg-stone-200 shadow-none"} flex items-center space-x-3 w-full transition-all py-3 px-2 rounded-xl`}>
      {icon === "ChartBarSquare" && <ChartBarSquareIcon className="w-7 h-7" />}
      {icon === "CreditCard" && <CreditCardIcon className="w-7 h-7" />}
      {icon === "ClipboardDocumentCheck" && (
        <ClipboardDocumentCheckIcon className="w-7 h-7" />
      )}
      {icon === "UserGroup" && <UserGroupIcon className="w-7 h-7" />}
